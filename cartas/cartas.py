...
class Jugador_partida:
    ...
    def elimina_duplicados(self):
        self.mano.elimina_duplicados()
    def ncartas(self):
        return self.mano.ncartas()
    def barajar(self):
        self.mano.barajar()
    def sacar(self, n=1):
        return self.mano.sacar(n)
    def get_nombre(self):
...
class Partida:
    ...
    def incorpora_jugador(self, jugador):
        ...
        mano = Mazo()
        nuevo_jugador = Jugador_partida(jugador,orden,0,mano)
        self.jugadores.append(nuevo_jugador)
    # Gestiona la partida
    def jugar(self):
        logging.basicConfig(filename="cartas.log")
        ...
        seguir = True
        for jugador in self.jugadores:
            jugador.elimina_duplicados()
            if jugador.ncartas() == 0:
                ganador = jugador
                seguir = False
                break
        turno = 1
        ...
        while seguir:
            ...
            print(f"Turno: {turno}")
            for jugador in self.jugadores:
                logging.info(jugador.get_nombre())
            ja = 0
            while ja<len(self.jugadores):
                js = ja+1
                if js>=len(self.jugadores):
                    js=0
                self.jugadores[ja].barajar()
                carta = self.jugadores[ja].sacar()
                if self.jugadores[ja].ncartas() == 0:
                    ganador = self.jugadores[ja]
                    seguir = False
                    break
                self.jugadores[js].recibir(carta)
                self.jugadores[js].elimina_duplicados()
                if self.jugadores[js].ncartas() == 0:
